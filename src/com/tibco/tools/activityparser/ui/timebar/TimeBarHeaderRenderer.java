...
/**
 * A header renderer that renders the label always on top of the supplied space
 * and doing a gradient fill of the background.
 * 
 * @author Jagdeesh Karicherla
 * @main author kliem
 */
public class TimeBarHeaderRenderer implements HeaderRenderer {
	private HeaderComponent _headerComponent;

	public TimeBarHeaderRenderer() {
	 ...
	 * {@inheritDoc}
	 */
	public JComponent getHeaderRendererComponent(TimeBarViewer tbv,
			TimeBarRowHeader value, boolean isSelected) {
	...
	public class HeaderComponent extends JComponent {
		...
		/**
		 * {@inheritDoc} The header renderer tries to get as much space as
		 * possible.
		 */
		...
		protected void paintComponent(Graphics graphics) {
			...
			GradientPaint gradientPaint = new GradientPaint(width / 2, 0,
					sColor, width / 2, height / 2, eColor, false);
			...
			// text
			Rectangle2D rect = graphics.getFontMetrics().getStringBounds("Q",
					graphics);
